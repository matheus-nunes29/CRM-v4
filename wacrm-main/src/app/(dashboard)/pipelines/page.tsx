...
export default function PipelinesPage() {
  ...
  const loadDeals = useCallback(
    async (pipelineId: string) => {
      const { data } = await supabase
        .from("deals")
        .select("*, contact:contacts(*, contact_tags(tags(id, name, color))), assignee:profiles!deals_assigned_to_fkey(*), custom_values:deal_custom_values(value, field:custom_fields(id, field_name, show_on_card))")
        .eq("pipeline_id", pipelineId)
        .order("created_at", { ascending: false });

      // Flatten nested contact_tags → tags onto the contact object
      const deals = (data ?? []).map((deal) => {
        if (deal.contact?.contact_tags) {
          deal.contact.tags = (deal.contact.contact_tags as { tags: unknown }[])
            .map((ct) => ct.tags)
            .filter(Boolean)
          delete deal.contact.contact_tags
        }
        return deal
      })
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      const autoStatus: DealStatus | undefined =
        ...
        targetStage?.fixed_role === 'won'  ? 'won'  : undefined

      setDeals((prev) =>
        prev.map((d) => {
          if (d.id !== dealId) return d
          const newStatus: DealStatus =
            autoStatus ?? ((d.status === 'won' || d.status === 'lost') ? 'open' : (d.status ?? 'open'))
          return { ...d, stage_id: newStageId, status: newStatus }
        }),
      );

      // Route through the server so CAPI, automations and conversation-close fire.
