...
function ResourcesProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ResourcesContext.Provider value={{ tags, members, templates, customFields, pipelines, pipelineStages }}>
      {children}
...
/** Pipeline dropdown by name, storing the pipeline's id. */
function PipelineSelect({
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className={SELECT_CLASS}>
      <option value="">Selecionar um pipeline…</option>
      {pipelines.map((p) => (
...
/** Stage dropdown filtered by the selected pipeline_id, storing the stage's id. */
function StageSelect({
  ...
  if (!pipelineId) {
    return (
      <p className="text-xs text-muted-foreground italic">Selecione um pipeline primeiro.</p>
    )
  }
  if (stages.length === 0) {
    return (
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className={SELECT_CLASS}>
      <option value="">Selecionar uma etapa…</option>
      {stages.map((s) => (
...
/** Title input for create_deal with variable chips. */
function DealTitleInput({
  value,
  onChange,
}: {
  value: string
  ...
  const VARS = [
    ...
    { label: "Texto da mensagem", token: "{{ message.text }}" },
  ]
  return (
    <div className="space-y-1.5">
      <Input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder="ex.: Lead de {{ contact.name }}"
        className="bg-muted text-foreground"
      />
...
function SendTemplateFields({
  ...
  return (
    <FieldBlock label="Template de envio">
      <select
        value={current}
        onChange={(e) => {
...
function StageEnteredTriggerConfig({
  ...
  const { pipelines } = useResources()
  // Derive the pipeline from the already-selected stage_id when editing
  const { pipelineStages } = useResources()
  const selectedStage = pipelineStages.find((s) => s.id === (config.stage_id as string))
  const [pipelineId, setPipelineId] = useState<string>(
    (config.pipeline_id as string) ?? selectedStage?.pipeline_id ?? ""
  )

  function handlePipelineChange(pid: string) {
    setPipelineId(pid)
    onChange({ ...config, pipeline_id: pid, stage_id: "" })
  }

  function handleStageChange(sid: string) {
    onChange({ ...config, pipeline_id: pipelineId, stage_id: sid })
  }
    ...
    <div className="space-y-3">
      <div>
        ...
        {pipelines.length > 0 ? (
          ...
          <p className="text-xs italic text-muted-foreground">Nenhuma pipeline cadastrada.</p>
        )}
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-muted-foreground">
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder="ex: preços, demonstração, falar com vendas"
          className="bg-muted text-foreground"
        />
      </div>
      <div>
