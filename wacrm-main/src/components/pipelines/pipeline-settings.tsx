...
function fixedSettingsSortKey(stage: import("@/types").PipelineStage): number {
...
export function PipelineSettings({
  ...
  const [autoCreateDeal, setAutoCreateDeal] = useState(pipeline.auto_create_deal ?? false);
  ...
  useEffect(() => {
    ...
    setAutoCreateDeal(pipeline.auto_create_deal ?? false);
    setLocalStages([...stages].sort((a, b) => fixedSettingsSortKey(a) - fixedSettingsSortKey(b)));
  ...
  function handleReorder(event: DragEndEvent) {
    ...
    const activeStage = localStages.find((s) => s.id === active.id);
    const overStage = localStages.find((s) => s.id === over.id);
    // Prevent moving fixed stages or moving a stage onto a fixed stage's position
    if (activeStage?.fixed_role || overStage?.fixed_role) return;
  ...
  async function handleSave() {
    ...
    // Fixed stages keep their canonical positions; regular stages get
    // index-based positions (filtered to exclude fixed stages for numbering).
    let regularIndex = 0;
    const stageRows = localStages.map((s) => {
      let position: number;
      if (s.fixed_role === 'new_lead') {
        position = -1;
      } else if (s.fixed_role === 'won') {
        position = 9998;
      } else if (s.fixed_role === 'lost') {
        position = 9999;
      } else {
        position = regularIndex++;
      }
      return {
        id: s.id,
    ...
    const [renameRes, stagesRes] = await Promise.all([
      supabase
        .from("pipelines")
        .update({ name: name.trim(), auto_create_deal: autoCreateDeal })
        .eq("id", pipeline.id),
  ...
  async function handleRemoveStage(stageId: string) {
    // Refuse to delete fixed stages.
    const stage = localStages.find((s) => s.id === stageId);
    if (stage?.fixed_role) {
      toast.error("Esta etapa é obrigatória e não pode ser excluída");
      return;
    }
            ...
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Isso vai arquivar todos os negócios neste pipeline. Esta ação não pode ser
                  desfeita.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 py-2">
              ...
              <div className="flex items-start gap-3 rounded-lg border border-border bg-muted/30 p-3">
                <button
                  type="button"
                  role="switch"
                  aria-checked={autoCreateDeal}
                  onClick={() => setAutoCreateDeal((v) => !v)}
                  className={`mt-0.5 relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                    autoCreateDeal ? "bg-primary" : "bg-muted-foreground/30"
                  }`}
                >
                  <span
                    className={`pointer-events-none block h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${
                      autoCreateDeal ? "translate-x-4" : "translate-x-0"
                    }`}
                  />
                </button>
                <div className="flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Quando um novo contato enviar mensagem via WhatsApp, cria
                    um negócio em "Novo Lead" neste pipeline.
                  </p>
                </div>
              </div>
...
function SortableStageRow({
  ...
  const isFixed = !!stage.fixed_role;
  ...
  return (
    ...
    >
      <button
        ...
        className={`touch-none text-muted-foreground ${isFixed ? 'pointer-events-none opacity-30' : 'cursor-grab hover:text-foreground active:cursor-grabbing'}`}
