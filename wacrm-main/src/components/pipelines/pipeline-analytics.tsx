...
export function PipelineAnalytics({ stages, deals }: PipelineAnalyticsProps) {
  ...
  const stats = useMemo(() => {
    ...
    // Use fixed_role from stage as source of truth; fall back to status for
    // deals that haven't been migrated to the fixed stages yet.
    const isWon  = (d: Deal) => stageById.get(d.stage_id)?.fixed_role === 'won'  || d.status === 'won'
    const isLost = (d: Deal) => stageById.get(d.stage_id)?.fixed_role === 'lost' || d.status === 'lost'

    const active   = deals.filter((d) => !isLost(d))
    const openDeals = active.filter((d) => !isWon(d))
    ...
    const wonThisMonth  = deals.filter((d) => isWon(d)  && thisMonth(d)).length;
    const lostThisMonth = deals.filter((d) => isLost(d) && thisMonth(d)).length;
