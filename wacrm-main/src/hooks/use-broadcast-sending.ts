...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function upsertCsvContacts(
    ...
    const uniqueByPhone = new Map<string, { phone: string; name?: string; company?: string }>();
    ...
    const missing = phones
      ...
      .map((phone) => ({
        ...
        company: uniqueByPhone.get(phone)?.company ?? null,
      }));
  ...
  async function createAndSendBroadcast(payload: BroadcastPayload): Promise<string> {
    ...
    try {
      ...
      // Delegate everything to the server — the send loop runs on Vercel,
      // so navigating away does not interrupt the broadcast.
      const res = await fetch('/api/whatsapp/broadcast/run', {
        method: 'POST',
