...
/**
 * Supabase origin(s) derived from the configured project URL instead of
 * hardcoded to `*.supabase.co` — so CSP stays correct whether this points
 * at Supabase Cloud or a self-hosted instance on any domain/IP.
 */
...
const SECURITY_HEADERS = [
  ...
  },
  {
    ...
    value: [
      ...
      // and the Supabase public-bucket audio/video the inbox renders.
      `media-src 'self' blob:${SUPABASE_ORIGIN ? ` ${SUPABASE_ORIGIN}` : ""}`,
      "font-src 'self' data:",
      // Supabase REST + realtime (WSS). All Meta/W-API/Evolution API
      // calls happen server-side, so those hosts don't belong here.
      `connect-src 'self'${SUPABASE_ORIGIN ? ` ${SUPABASE_ORIGIN} ${SUPABASE_WS_ORIGIN}` : ""}`,
